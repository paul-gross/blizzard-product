...
| `epic:ci-feedback` | Routing CI and review results back into the owning session, capped feedback rounds. | The merge-queue machinery it feeds into exists. |
...
| `epic:hub` (remote slice) | Off-machine hub, multiple runners — a [`milestone:centralized-hub`](./milestones.md) spoke. | The separation slice (colocated hub, runner↔hub protocol) shipped in the MVP. |
...
| `epic:batching` | Execution units, the heuristic packer, the LLM planner, conflict packing, batch failure semantics. | **Parked** by design; keep captured, don't schedule. |
...
|------|---------------|--------|
| `epic:board` (remote slice) | Human auth, roles, capability-aware session UI, runner SSO — the auth prerequisite of the board's remote slice. | #89 (epic), #91–#96 |
