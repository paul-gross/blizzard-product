...
3. **Filed issues** — file the GitHub epic issue and its child issues (the `/wg-issue` flow), each citing `epic:<slug>`. Move the epic's row to **In flight**. The issue tracker is the factory's intake queue, not its memory: file only work that is startable, keep the open count small.
...
- **Closing issues is the execution record.** When an epic's slice completes, record it in [delivered.md](./delivered.md) under each milestone it served (or in the outside-the-milestones section) and drop it from epics.md; when a milestone is reached, its delivered.md section is the record and its row leaves milestones.md. Delivered entries keep their links to plans.
