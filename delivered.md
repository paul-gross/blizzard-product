...
|------|-----------------|-------|
| `epic:hub` | separation slice | Colocated hub + runner↔hub protocol, queue, HTTP API + SSE, merge queue. Remote slice is in the [epic registry](./epics.md). |
...
| `epic:ask-answer` | hub slice | Question rows at the hub, ask/answer CLI verbs. Remote slice is in the [epic registry](./epics.md). |
| `epic:board` | local slice | Hub-served web app: fleet observability, queue prioritization, chunk grouping. Remote/auth slice is in flight in the [epic registry](./epics.md). |
...
| `epic:migration` | core | Explicit migration, `migration_target` auto-drift, retire/re-enable, graph metadata edits (#101, #120, #124 …). Correctness tail is in flight in the [epic registry](./epics.md). |
