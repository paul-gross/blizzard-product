...
| `epic:adapters`   | full             | Harness adapters (spawn / resume / verdict), hooks, selftest, human takeover. Built from its [plan](./plans/adapters/claude-code.md).                                                                   |
...
| `epic:board`      | local slice      | Hub-served web app: fleet observability, queue prioritization, chunk grouping. Remote/auth slice delivered with `milestone:centralized-hub` below. Built from its [plan](./plans/board-local/index.md). |
...
| `epic:board`      | remote (auth) slice | Provider login, viewer and operator roles, a capability-aware UI, runner single sign-on (#89). Its reach half was extracted to `epic:pwa`. Built from its [plan](./plans/board-remote/index.md).                                                                                                                                                             |
...
| `epic:board`        | chunk-detail slice  | Routed chunk detail page with General/Artifacts tabs; dock artifacts became links that land pre-selected in one shared renderer (#199). Built from its [plan](./plans/chunk-detail/index.md).                                                                                                                                                           |
| `epic:board`        | mobile slice        | The board complete from a phone: every read and every action the operator has at a desk, present and usable at thumb size. Landed incrementally as layout and control tuning rather than from a plan, and closed on the operator's daily use of it. Closes `epic:board` — the home-screen shell and phone notifications it once trailed are `epic:pwa`. |
