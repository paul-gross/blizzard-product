...
registry aligns the priority of the plans), from its epic plan's `slices` frontmatter (a slice plan), or from
`delivered.md` (completed work). Every live epic's row links its epic plan. A plan referenced from none of these is
...
- **Work-tracking folders of any kind** — blizzard's ready queue is GitHub and its execution record is closed issues
  plus `delivered.md`. The one piece of execution state kept here is the coarse per-slice status in each epic plan's
  frontmatter; issue-level progress is never mirrored in files or directories.
