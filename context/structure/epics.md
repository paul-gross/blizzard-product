...
There is **no status** in this file. Whether an epic's work is filed, started, or ongoing is not tracked here — each
slice's status lives in its epic plan's frontmatter ([structure/plans.md](./plans.md)), repeated in the milestone epic
charts, with issue-level progress in GitHub, and a fully-landed slice resolves in `delivered.md`. The registry never
carries issue numbers, and it has no `Horizon`/`In flight` split — a single flat, priority-ordered table is the whole
shape.
