...
You have two inputs: the `survey` asset from this run, and the findings already live on this routine in this run's
scope. Fetch the latter through the CLI — the routine and the scope are named in the chunk's work item. What comes back
is your scope's bucket, not the routine's whole set: findings recorded under other scopes are deliberately not in front
of you. Read both before you write anything.
...
The shape is in the `findings` doc. Every transformation must name a `fin_` id that is actually live on this routine;
the delivery step rejects the artifact if it does not.
