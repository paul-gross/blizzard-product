...
A hub work item carries a per-source monotonic integer `ref` (the `42` in `hub:42`, never reused), a `title`, a markdown `body` that is the work's full spec, an `author`, an advisory `stated_priority` (`low`/`normal`/`high`), creation and last-edit instants, and a closure — `delivered` when its chunk lands, `withdrawn` when it is deleted. The author is structured, not a string: kind `user` with the hub user's login, or kind `fleet` with the runner, chunk, and node resolved server-side from the caller's principal and live lease. Comments stay out of the table: the fetch seam returns an empty list and every consumer is satisfied; an append-only comments table slots in behind the same seam the day a real need appears.
...
- **The fleet creation twin.** `POST /api/fleet/work-sources/hub/items` on the fleet router: a runner principal files an item and the hub stamps its authorship — runner, chunk, node — from the lease, mirroring how the fleet router already twins the work-item read. This route is the seam the producer epics call; it ships here so filing work is never the part a producer has to build.
...
Not the producers: no node authors work yet — the delivery-review pass and the retrospective's finding-to-item rewiring are their own epics, and this one's fleet route is the seam they will call. Not auto-promotion: a stated priority is advice for the triaging human, and the promote gate holds against fleet-authored work exactly as it holds against forge-born work. Not comments, until something needs them. Not forge editing: the capability gate leaves that door framed but unopened.
