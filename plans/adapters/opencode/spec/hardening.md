...
not moving, which the [harness selection specification](../../harness-selection.md) settles as a question the hub does
not answer.
...
Capability advertisement and eligibility change the hub-to-runner wire, so the same steps extend `blizzard-mock`. The
...
The subscription work extends the companion in the same change. The mock runner emits slug-carrying per-subscription
...
The compatibility step also adds `blizzard:manual-opencode-compatibility` to the application verifiability matrix and
